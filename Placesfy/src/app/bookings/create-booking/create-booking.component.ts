...
export class CreateBookingComponent implements OnInit {
  ...
  @Input() selectedModel: "select" | "random";
  ...
  ngOnInit() {
    ...
    if(this.selectedModel === "random") {
      ...
      this.endDate = new Date(new Date(this.startDate).getTime() + Math.random() * (new Date(this.startDate).getTime() + 6*24*60*1000) - new Date(this.startDate).getTime()).toISOString();
    }
